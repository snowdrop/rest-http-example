...
First, we need to tell Maven that we want to use this plugin. This is accomplished in the parent POM of our example, which is declared as:
```xml
<parent>
...
...
  <properties>
    ...
    <fabric8.generator.from>registry.access.redhat.com/ubi8/openjdk-8:1.3</fabric8.generator.from> #<2>
...
https://access.redhat.com/containers/?tab=overview&platform=openshift#/registry.access.redhat.com/ubi8/openjdk-8[Red Hat supported OpenJDK 8 image] since our application is, at its code, a Java application. The example's parent specifies that version `1.2` should be used but this property is overridden in the example itself to use a more recent version. We will look later at how this is done in our example (essentially, redefining the `fabric8.generator.from` property but with a twist)!
...
Now that we've seen the configuration of the plugin in the booster parent, let's look at how it's used in the example itself:

```xml
...
  <properties>
    ...
    <ubi.version>1.3</ubi.version> #<2>
    <fabric8.generator.from>
      registry.access.redhat.com/ubi8/openjdk-8:${ubi.version} #<3>
    </fabric8.generator.from>
  </properties>
  ...
  <profile>
...
<2> Earlier, we said that we would redefine the version of the base image used to perform our builds. This is how we do it: instead of re-defining the whole `fabric8.generator.from` property, we introduce a new `ubi.version` property, which we set to the desired value and
...
```
+ kubernetes
\__ rest-http-deployment.yml
 |_ rest-http-svc.yml
- kubernetes.json
- kubernetes.yml
+ openshift
 \__ rest-http-deploymentconfig.yml
  |_ rest-http-route.yml
  |_ rest-http-svc.yml
- openshift.json
- openshift.yml
```
...
This works because FMP generated the `rest-http` `Route` automatically for you as follows:
...
Let's now look at some the `rest-http-svc.yml` file, the `Service` definition:
...
metadata:
  annotations:  #<2>
    ...
    fabric8.io/scm-con-url: scm:git:https://github.com/openshiftio/booster-parent.git/rest-http
    prometheus.io/port: "9779"
    fabric8.io/scm-url: https://github.com/openshiftio/rest-http
    fabric8.io/iconUrl: img/icons/spring-boot.svg
    fabric8.io/git-branch: guide
    prometheus.io/scrape: "true"
    fabric8.io/scm-devcon-url: scm:git:git:@github.com:openshiftio/booster-parent.git/rest-http
...
<6> Note also, that we add an `app` label with the `rest-http` value, label that is added to all the generated resources to mark them as being part of the same application.
...
metadata:
  annotations:
    fabric8.io/git-commit: 1fc0c37f21be5db9a77f93e42382583b32a588a7
    fabric8.io/metrics-path: dashboard/file/kubernetes-pods.json/?var-project=rest-http&var-version=1.5.14-1-SNAPSHOT
    fabric8.io/scm-con-url: scm:git:https://github.com/openshiftio/booster-parent.git/rest-http
    fabric8.io/scm-url: https://github.com/openshiftio/rest-http
    fabric8.io/iconUrl: img/icons/spring-boot.svg
    fabric8.io/git-branch: guide
